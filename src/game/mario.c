    ...
    switch (action) {
        ...
        case ACT_LONG_JUMP:
            ...
            if ((m->forwardVel *= 1.5f) > 48.0f) {
                m->forwardVel = 48.0f;
            } else if (m->forwardVel < 0.0f) {  // Cheat: free BLJs
                m->vel[1] = 0.0f;
            }
            break;
...
void update_mario_health(struct MarioState *m) {
    ...
    if (m->health >= 0x100) {
        ...
        if (((u32) m->healCounter | (u32) m->hurtCounter) == 0) {
            if ((m->input & INPUT_IN_POISON_GAS) && ((m->action & ACT_FLAG_INTANGIBLE) == 0)) {
                if (((m->flags & MARIO_METAL_CAP) == 0) && (gDebugLevelSelect == 0)) {
                    // m->health -= 4;  // Cheat: invincible Mario
                }
            } else {
                if ((m->action & ACT_FLAG_SWIMMING) && ((m->action & ACT_FLAG_INTANGIBLE) == 0)) {
                    ...
                    if ((m->pos[1] >= (m->waterLevel - 140)) && !terrainIsSnow) {
                        ...
                        // m->health -= (terrainIsSnow ? 3 : 1);  // Cheat: invincible Mario
                    }
                }
            }
        }
        ...
        if (m->hurtCounter > 0) {
            // m->health -= 0x40;  // Cheat: invincible Mario
            m->hurtCounter--;
        }
